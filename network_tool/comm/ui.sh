...
function checkInputHostname(){
    local _in_name=$1
    if test -z `echo $_in_name | grep -E '^(cc|nc|nn|autodeploy)\S*\..+'`; then
        return 0
    else
        return 1
    fi
...
function UI_setHostName(){
    echo ""
    echo "The hostname should be named below:"
    echo "  control node: cc{num}.domain.domain"
    echo "  compute node: nc{num}.domain.domain"
    echo "  network node: nn{num}.domain.domain"
    echo "  deploy  node: autodeploy.domain.domain"
    echo ""
     _in_name=$(UI_getInput "Please enter the Hostname: ")
	checkInputHostname $_in_name
	if [ $? -eq 0 ];then
	    echo "Input error! Please input the Hostname: "
	    UI_pleaseWait
	    clear
	    UI_setHostName
	fi
